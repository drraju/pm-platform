...
export class ProjectExportService {
  ...
  async exportProject(projectId: string, actor: { userId: string; roleId: string }, format = 'excel') {
    ...
    const [taskDependencies, executionHistory, documents] = await Promise.all([
      taskIds.length ? this.dependencies.find({ where: [{ predecessorTaskId: In(taskIds) }, { successorTaskId: In(taskIds) }] }) : Promise.resolve([]),
      this.executionUpdates.find({ where: { projectId }, order: { createdAt: 'ASC' } }),
      this.documents.findProjectDocuments(projectId),
    ]);
